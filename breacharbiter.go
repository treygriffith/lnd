...
func (b *breachArbiter) Start() error {
	...
	// database.
	if err := b.waitForPendingCloseConfs(currentHeight); err != nil {
		return err
	}
...
func newRetributionInfo(chanPoint *wire.OutPoint,
	...
	// CommitmentRevoke, since we will be using a revoke key, and
	// withdrawing the funds from the commitment transaction immediately.
