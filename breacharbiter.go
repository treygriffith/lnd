...
func newRetributionInfo(chanPoint *wire.OutPoint,
	...
	breachedOutputs := make([]breachedOutput, nHtlcs+2)

	// In the case that either commitment output is dust, its sign
	// descriptor will be set to nil. Since either may be ignored, we keep
	// track of the offset at which to insert subsequent non-dust outputs.
	var offset int
	...
	if breachInfo.LocalOutputSignDesc != nil {
		breachedOutputs[offset] = makeBreachedOutput(
	...
	for i, breachedHtlc := range breachInfo.HtlcRetributions {
		...
		breachedOutputs[offset+i] = makeBreachedOutput(
			&breachInfo.HtlcRetributions[i].OutPoint,
			htlcWitnessType,
			&breachInfo.HtlcRetributions[i].SignDesc)
	}

	// Trim the list of breached outputs in case either or both commitment
	// outputs were filtered as dust.
	breachedOutputs = breachedOutputs[:offset+nHtlcs]
